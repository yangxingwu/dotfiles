...
Zellij 中有两种方式复制 pane 中的文本：

**方式一：通过 $EDITOR 复制（键盘流）**

1. `Ctrl+s` — 进入 Scroll 模式
2. 按 `e` — 将整个滚动缓冲区打开到 `$EDITOR`（即 nvim）
3. 在编辑器中用编辑器自身的方式选择和复制（vim 中 `v` 进入 Visual 模式选中文本，`"+y` 复制到系统剪贴板）
4. 退出编辑器（vim 中 `:q`）

注意：需要你的 `$EDITOR` 正确配置了系统剪贴板集成。Neovim 中确认 `clipboard = "unnamedplus"` 已设置，否则复制内容只留在编辑器内部寄存器中。

**方式二：鼠标选择**

| 操作 | 效果 |
|---|---|
| 直接鼠标拖选 | 复制到 Zellij 内部剪贴板（按系统粘贴键可粘贴） |
| `Shift` + 鼠标拖选 | 绕过 Zellij，使用终端模拟器原生选择，复制到系统剪贴板 |

对于快速复制一小段文本，`Shift` + 鼠标拖选是最省事的方式。
